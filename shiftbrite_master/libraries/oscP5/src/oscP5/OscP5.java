...
public class OscP5 implements UdpPacketListener, TcpPacketListener {
	...
	public OscP5(final Object theParent, final OscProperties theProperties) {
		...
		if (_myOscProperties.networkProtocol() == OscProperties.MULTICAST) {
			Logger.printInfo("OscP5", "is joining a multicast group @ "
					+ _myOscProperties.remoteAddress().address() + ":"
					+ _myOscProperties.remoteAddress().port());
		} else {
			Logger.printInfo("OscP5", "is running. you (" + ip()
					+ ") are listening @ port "
					+ _myOscProperties.remoteAddress().port());
		}
	}
	 ...
	 * @usage Application
	 */
	public OscP5(final Object theParent, final String theAddress,
			final int thePort, final int theMode) {
		welcome();
		parent = theParent;
	...
	public OscP5(final Object theParent, final int theReceiveAtPort,
		...
		case (MULTICAST):
			Logger
					.printWarning(
							"OscP5",
							"please specify a multicast address. use "
									+ "OscP5(Object theObject, String theMulticastAddress, int thePort, int theMode)");
			break;
		}
		isEventMethod = checkEventMethod();
	}
	...
	private void welcome() {
		System.out
				.println("OscP5 "
						+ VERSION
						+ " "
						+ "infos, comments, questions at http://www.sojamo.de/oscP5\n\n");
	}
	...
	private void initUDP(final int theReceiveAtPort) {
		...
		_myOscNetManager.addUdpListener(this);
		Logger.printInfo("OscP5", "is running. you (" + ip()
				+ ") are listening @ port " + theReceiveAtPort);
	}
	...
	private boolean checkEventMethod() {
		...
		try {
			...
			for (int i = 0; i < myMethods.length; i++) {
				if (myMethods[i].getName().indexOf(
						_myOscProperties.eventMethod()) != -1) {
					Class<?>[] myClasses = myMethods[i].getParameterTypes();
					if (myClasses.length == 1) {
						_myEventClass = myClasses[0];
						isOscIn = ((_myEventClass.toString()).indexOf("OscIn") != -1) ? true
								: false;
						break;
					}
				}
		...
		if (theOscMessage.addrPattern().startsWith("/sys/ping")) {
			...
			if (theOscMessage.tcpConnection() != null) {
				if (theOscMessage.checkTypetag("s")) {
					theOscMessage.tcpConnection().setName(
	...
	public void disconnect(final NetAddress theHost) {
		if (theHost.isvalid() && theHost.name.length() > 1) {
			String myAddrPattern = "/sys/disconnect/" + theHost.name + "/"
					+ theHost.port();
	 ...
	 */
	public void send(final String theAddrPattern, final Object[] theArguments,
			final String theAddress, int thePort) {
		_myOscNetManager
	 ...
	 * @deprecated
	 */
	public void send(final OscPacket thePacket, final String theIpAddress,
			final int thePort) {
	...
	public static void flush(final String theAddrPattern,
			final Object[] theArguments, final NetAddress theNetAddress) {
		flush((new OscMessage(theAddrPattern, theArguments)).getBytes(),
				theNetAddress);
	}
	...
	public static void flush(final byte[] theBytes,
		...
		try {
			...
			DatagramPacket myPacket = new DatagramPacket(theBytes,
					theBytes.length, theNetAddress.inetaddress(), theNetAddress
							.port());
			mySocket.send(myPacket);
		} catch (SocketException e) {
			Logger.printError("OscP5.openSocket", "cant create socket "
					+ e.getMessage());
		} catch (IOException e) {
			Logger.printError("OscP5.openSocket",
	 ...
	 * @deprecated
	 */
	public static void flush(final byte[] theBytes, final String theAddress,
			final int thePort) {
	 ...
	 */
	public static void flush(final OscMessage theOscMessage,
			final String theAddress, final int thePort) {
	 ...
	 * @invisible
	 */
	public OscP5(final Object theParent, final String theHost,
			final int theSendToPort, final int theReceiveAtPort,
